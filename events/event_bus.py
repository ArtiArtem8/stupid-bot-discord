...
class EventBus:
    ...
    async def publish(self, event: BaseEvent) -> None:
        ...
        if not handlers:
            logger.debug("No handlers for %s", event.event_name)
            return
        ...
        try:
            ...
            logger.exception("Error publishing event %s", event.event_name)
    ...
    async def _process_handler(self, handler: EventHandler, event: BaseEvent) -> None:
        try:
            ...
            logger.exception(
                "Error in handler %s for %s", handler.__name__, event.event_name
            )
