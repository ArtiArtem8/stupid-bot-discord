    ...
    async def join(
        ...
        try:
            if voice_client and isinstance(voice_client, MusicPlayer):
                ...
                if not self.is_player_usable(voice_client):
                    await self.detach_stale_voice_client(guild, voice_client)
                    return VoiceCheckResult.MUSIC_SERVICE_UNAVAILABLE, None
                try:
                    await voice_client.move_to(channel, timeout=5.0)
            ...
            if isinstance(guild.voice_client, MusicPlayer):
                await self._handle_voice_client_io_failure(
                    guild, guild.voice_client, exc
                )
            else:
                await self._detach_voice_client_after_failed_connect(guild)
