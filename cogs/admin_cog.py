...
class AdminCog(commands.Cog):
    ...
    async def unblock(
        ...
        try:
            ...
            self.logger.info(f"Error: {e}", exc_info=1)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        try:
            ...
            if history:
                embed.add_field(
                    ...
                    value="\n".join(history)[:1024],  # Limit to field max size
                    inline=False,
                )
            ...
            self.logger.error(f"Something went wrong. {e}", exc_info=1)
